...
from utils import (
    ...
    get_opposite_model,
    extract_claude_transcript_data,
)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Override the default Model B selection.",
    )
    ...
    try:
        ...
        if args.model_id:
            model_b_id = args.model_id
        else:
            model_b_id = get_opposite_model(model_a_id)
        if model_b_id == model_a_id:
            raise Exception(
                "Model B must differ from Model A. Choose a different --model-id value."
            )
